...
const CloudinaryUpload = ({ setUrl, setName }) => {
  const uploadImageWidget = () => {
    let myUploadWidget = openUploadWidget(
      ...
      function (error, result) {
        // 📝this 'result.info' prop will be an object with all the properties of uploaded item, eg uploaded 'secure_url' which will be hosted url of uploaded item
        if (!error && result.event === "success") {
          ...
          if (error) console.log(error); // can not upload songs
        }
      }
    );
    myUploadWidget.open();
  ...
  return (
    <button
      className="px-4 py-3 font-semibold text-black bg-white rounded-full"
