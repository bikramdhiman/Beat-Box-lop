...
export default function MyMusic() {
  ...
  useEffect(() => {
    // imp note : we can not make useEffect async directly, then how can me mak makeAuth.. function async ?, the solution is that we can make another function inside the useEffect and make it async then inside this function call teh makeAuth.. function to call API at backend to return some response
  ...
  return (
    <LoggedInContainer curActiveScreen={"myMusic"}>
      {" "}
      {/* repeatitive navbar,sidebar,songBar are wrapped in this component */}
      {/* children begins */}
      {/* song cards container */}
      <div className="pt-8 pb-4 pl-2 text-xl font-semibold text-left">
        My Songs
      </div>
      <div className="space-y-3 overflow-auto">
        {
          ...
          songData.map((item) => {
            // 'item' is individual songs object
