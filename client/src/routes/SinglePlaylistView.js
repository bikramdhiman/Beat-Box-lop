...
export default function SinglePlaylistView() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      ); // APi will be called for something like playlist/get/playlistasdjkn24553 where end one is the playlist id for which details json we want to fetch frmo backend
    ...
    <LoggedInContainer curActiveScreen={"library"}>
      {
        ...
        playlistDetails._id && (
          <div>
            <div className="py-6 text-xl font-bold text-left">
              {playlistDetails.name}{" "}
              {/*fetch the name of playlist from state */}
            </div>
            ...
            <div className="pt-5 space-y-1">
              ...
              {playlistDetails.songs.map((item) => {
                return (
                  <SingleSongCard info={item} key={JSON.stringify(item)} />
                ); // this item is the each songs object with all its details
              })}
            </div>
          </div>
        )
