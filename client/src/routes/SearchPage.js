...
export default function SearchPage() {
  ...
  const searchSong = async () => {
    ...
    ); // this will call teh backend and will search and get all the songs matching the keyword 'searchText' and then returns a object
  ...
  return (
    <LoggedInContainer curActiveScreen={"search"}>
      <div className="w-full py-6 text-left">
        <div
          className={`w-1/3 flex justify-center items-center p-3 rounded-full bg-gray-800 px-5 space-x-3 ${
        ...
        >
          ...
          <input
            ...
            onKeyDown={(e) => {
              // this func is a prop of input field, which will hit whenever user press any key
              // console.log(e);
              if (e.key === "Enter") {
                // when user press enter key, then only start the search func
                searchSong();
              }
            }}
          />
        ...
        {songData.length > 0 ? ( // conditionally render this below part only when user has typed and entered something
          ...
          <div className="pt-10 pl-5 text-gray-400">Nothing to show here.</div>
        )}
      </div>
    </LoggedInContainer>
  );
