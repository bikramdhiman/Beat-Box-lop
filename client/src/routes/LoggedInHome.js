...
export default function Home() {
  ...
  return (
    ...
    <LoggedInContainer curActiveScreen={"home"}>
      {" "}
      {/* now why we passed this curActiveScreen = home, bacause now wheverever this component renderes, this value 'home'is passed to the Logged In container.js and there that screen becomes active */}
      {/* <PlaylistView titleText={"Focus"} cardsData={focusCardsData}/>
            <PlaylistView titleText={"More Playlists"} cardsData={spotifyCardsPlaylist}/>  */}
      {/* updation : */}
      <div className="py-5 text-xl font-semibold text-left">Top Songs</div>
      <div className="flex flex-wrap items-center w-full justify-left">
        {songsData.map((songObject) => {
          return (
            <SongCard songObject={songObject} setCurrentSong={setCurrentSong} />
          );
        })}
      </div>
    </LoggedInContainer>
...
const SongCard = ({ songObject, setCurrentSong }) => {
  return (
    <div
      className="flex-col items-center justify-center w-1/6 p-2 mx-4 my-4 cursor-pointer"
      onClick={() => {
        setCurrentSong(songObject);
      }}
    >
      <div className="w-full">
        <img src={songObject.thumbnail} className="w-full rounded-xl" />
      </div>
      <div className="pt-2 pl-1 font-semibold text-left text-gray-300 text">
        {" "}
        {songObject.name}{" "}
      </div>
      <div className="pl-1 text-sm text-left text-gray-500">
        {" "}
        {songObject.artist.firstName + " " + songObject.artist.lastName}
      </div>
    </div>
  );
};
...
const Card = ({ title, description, imgUrl }) => {
  return (
    <div className="w-1/5 p-4 duration-200 bg-black border border-gray-200 rounded-lg cursor-pointer bg-opacity-40 border-opacity-10 hover:bg-yellow-100 hover:bg-opacity-5 delay-50 hover:border-gray-600">
      <div className="pb-4 bt-2">
        <img className="w-full rounded-md" src={imgUrl} alt="playlist cover" />
      </div>
