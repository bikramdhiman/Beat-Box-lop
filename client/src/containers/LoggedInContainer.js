...
export default function LoggedInContainer({ children, curActiveScreen }) {
  ...
  const {
    ...
    isPaused,
    setIsPaused,
  } = useContext(songContext); // this currentSong will have value of obj 'info' of song which is global
  ...
  const playSound = () => {
    if (!soundPlayed) {
      // if soundPlayed is null do not play it
      return;
    }
  ...
  const changeSong = (songSrc) => {
    if (soundPlayed) {
      // if a song is alrady playing then stop it first
      soundPlayed.stop();
    }
  ...
  const addSongToPlaylist = async (playlistId) => {
    ...
    const response = await makeAuthenticatedPOSTRequest(
      "/playlist/add/song",
      payLoad
    ); // add song to playlist by calling Post api at backend

    if (response._id)
      // if song successfully added to the desired playlist, then close the AddToPlaylist Modal
  ...
  return (
    <div className="w-full h-full bg-app-black">
      ...
      {/* we passed the closeModal func as prop to this comp and inside that comp, when user clicks on outter div of the createplaylistModal this state will set to false */}
      ...
      <div className={`${currentSong ? "h-9/10" : "h-full"} w-full flex `}>
        {" "}
        {/* when a curr song is null means no song played by user then we this screen will be full (no song bar at bottom), if not null then this screen will take height of 90% of screnn and rest 10%is for songBar*/}
          ...
          <div className="flex items-center justify-end w-full bg-black navbar h-1/10 bg-opacity-40">
            <div className="flex items-center w-1/2 h-full">
              ...
              <div className="flex items-center justify-around w-3/5 h-full ">
                ...
                <div
                  className="px-2 py-2 font-semibold text-black duration-150 delay-100 bg-white rounded cursor-pointer hover:opacity-90"
                  onClick={() => {
                    navigate("/uploadSong");
                  }}
                >
                ...
                <div
                  className="flex items-center justify-center px-3 py-2 space-x-2 text-sm text-white duration-200 delay-100 border border-gray-500 rounded cursor-pointer hover:bg-white hover:bg-opacity-20 hover:font-bold hover:font-semibold "
                  onClick={() => {
                    // removeCookie();
      ...
      {/* song play/pause bar*/}
      {
        ...
        currentSong && (
          <div className="flex items-center w-full bg-black h-1/10 bg-opacity-30 ">
            {/* this will be left part of song play bar */}
            <div className="flex items-center w-1/4 h-full p-4">
              <img
              ...
              />
              <div className="pl-4">
                <div className="text-sm text-left text-white cursor-pointer hover:underline max-w-max">
                  {currentSong.name}
                </div>
                <div className="text-xs text-left text-gray-400 cursor-pointer hover:underline max-w-max">
                  {currentSong.artist.firstName +
                    " " +
                    currentSong.artist.lastName}{" "}
                  {/*because currentSong.artist is an object with all info of artist that fetched did with .populate() function in backend*/}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center w-2/4 h-full ">
              <div className="flex items-center justify-center justify-between w-1/3 h-full">
                ...
                <Icon /* Play/Pause icon*/
                  icon={
                    isPaused ? "carbon:play-outline" : "carbon:pause-outline"
                  }
                  fontSize={35}
                  className="text-gray-400 cursor-pointer hover:text-white"
                  onClick={() => {
              ...
              <div>{/* song progress bar */}</div>
            </div>

            <div className="flex items-center justify-end w-1/4 h-full pr-5 space-x-3">
              <Icon
                ...
                }} // open the add to playlist modal when user clicks on this playlist icon at the songBar at bottom right
              />
              <Icon
